...
#define UNREACHABLE_RETURN Q_ASSERT(0)
...
{
    asCScriptNode *node = new asCScriptNode(type);
...
asCScriptNode *asCParser::ParseDataType(bool allowVariableType, bool allowAuto)
{
	...
	if( !IsDataType(t1) && !(allowVariableType && t1.type == ttQuestion) && !(allowAuto && t1.type == ttAuto) )
	{
		...
		{
			tempString = script->code.mid(t1.pos, t1.length);
			QString errMsg = QString::asprintf(TXT_IDENTIFIER_s_NOT_DATA_TYPE, qUtf8Printable(tempString));
			Error(errMsg, &t1);
		}
	...
	for(;;)
	{
		...
		else if( t.type == ttCloseParenthesis)
		{
			...
			else
			{
				// Wrong syntax
				RewindTo(&t);
				QString str = QString::asprintf(TXT_UNEXPECTED_TOKEN_s, ")");
				Error(str, &t);
				return node;
			}
		}
...
void asCParser::GetToken(sToken *token)
{
	...
	do
	{
		if( sourcePos >= sourceLength )
		{
			...
			token->type = m_tok.GetToken(script->code.data() + sourcePos, sourceLength - sourcePos, &token->length);
...
void asCParser::Error(const QString &text, sToken *token)
{
	RewindTo(token);
...
void asCParser::Info(const QString &text, sToken *token)
{
	RewindTo(token);
...
bool asCParser::IsDataType(const sToken &token)
{
	if( token.type == ttIdentifier )
	{
		...
		if( checkValidTypes )
		{
			...
			tempString = script->code.mid(token.pos, token.length);
...
QString asCParser::ExpectedToken(const char *token)
{
	QString str = QString::asprintf(TXT_EXPECTED_s, token);

	return str;
}

QString asCParser::ExpectedTokens(const char *t1, const char *t2)
{
	QString str = QString::asprintf(TXT_EXPECTED_s_OR_s, t1, t2);

	return str;
}

QString asCParser::ExpectedOneOf(int *tokens, int count)
{
	QString str;
...
QString asCParser::ExpectedOneOf(const char **tokens, int count)
{
	QString str;
...
QString asCParser::InsteadFound(sToken &t)
{
	QString str;
	if( t.type == ttIdentifier )
	{
		QString id = script->code.mid(t.pos, t.length);
		str = QString::asprintf(TXT_INSTEAD_FOUND_IDENTIFIER_s, qUtf8Printable(id));
	}
	else if( t.type >= ttIf )
		str = QString::asprintf(TXT_INSTEAD_FOUND_KEYWORD_s, asCTokenizer::GetDefinition(t.type));
	else
		str = QString::asprintf(TXT_INSTEAD_FOUND_s, asCTokenizer::GetDefinition(t.type));
...
asCScriptNode *asCParser::ParseListPattern()
{
	...
	while( !isSyntaxError )
	{
		...
		else if( t1.type == ttIdentifier && (IdentifierIs(t1, "repeat") || IdentifierIs(t1, "repeat_same")) )
		{
			if( !isBeginning )
			{
				QString token = script->code.mid(t1.pos, t1.length);
				QString msg = QString::asprintf(TXT_UNEXPECTED_TOKEN_s, qUtf8Printable(token));
				Error(msg, &t1);
			}
			RewindTo(&t1);
			node->AddChildLast(ParseIdentifier());
...
bool asCParser::FindTokenAfterType(sToken &nextToken)
{
	...
	GetToken(&t2);
	while (t2.type == ttAmp || t2.type == ttOpenBracket)
	{
...
asCScriptNode *asCParser::ParseArgList(bool withParenthesis)
{
	...
	if( t1.type == ttCloseParenthesis || t1.type == ttCloseBracket )
	{
		if( withParenthesis )
		{
			...
			{
				QString str = QString::asprintf(TXT_UNEXPECTED_TOKEN_s, asCTokenizer::GetDefinition(ttCloseBracket));
...
int asCParser::ParseFunction(asCScriptCode* in_script)
{
	Reset();
...
int asCParser::ParseFunctionStatement(asCScriptCode* in_script)
{
	Reset();
	...
	for(;;)
	{
		while( !isSyntaxError )
		{
			...
			else
			{
				...
				QString str = QString::asprintf(TXT_UNEXPECTED_TOKEN_s, t);
...
asCScriptNode *asCParser::ParseFuncDef()
{
	...
	if( t1.type != ttIdentifier )
	{
		Error(asCTokenizer::GetDefinition(ttIdentifier), &t1);
		return node;
	}
...
int asCParser::ParseVarInit(asCScriptCode *in_script, asCScriptNode *in_init)
{
	...
	{
		QString msg = QString::asprintf(TXT_UNEXPECTED_TOKEN_s, asCTokenizer::GetDefinition(t.type));
		Error(msg, &t);
	}
