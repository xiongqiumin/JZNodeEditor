...
int as_tokencmp(const QChar *s1, const char *s2, int len)
{
    for (int i = 0; i < len; i++)
    {
        if (s1[i] != s2[i])
...
{	
	...
	for( uint32_t n = 0; n < numTokenWords; n++ )
	{
		...
		if( !keywordTable[start] )
		{
			...
			keywordTable[start] = new const sTokenWord*[32];
...
bool asCTokenizer::IsDigitInRadix(QChar qch, int radix) const
{
...
eTokenType asCTokenizer::GetToken(const QChar *source, size_t sourceLength, size_t *tokenLength, asETokenClass *tc) const
{
	Q_ASSERT(source != 0);
	Q_ASSERT(tokenLength != 0);
...
asETokenClass asCTokenizer::ParseToken(const QChar *source, size_t sourceLength, size_t &tokenLength, eTokenType &tokenType) const
{
...
bool asCTokenizer::IsWhiteSpace(const QChar *source, size_t sourceLength, size_t &tokenLength, eTokenType &tokenType) const
{
	// Treat UTF8 byte-order-mark (EF BB BF) as whitespace
	if( sourceLength >= 3 && 
		uint8_t(source[0].toLatin1()) == 0xEFu &&
		uint8_t(source[1].toLatin1()) == 0xBBu &&
		uint8_t(source[2].toLatin1()) == 0xBFu )
	{
...
bool asCTokenizer::IsComment(const QChar *source, size_t sourceLength, size_t &tokenLength, eTokenType &tokenType) const
{
	if( sourceLength < 2 )
		return false;
...
bool asCTokenizer::IsConstant(const QChar *source, size_t sourceLength, size_t &tokenLength, eTokenType &tokenType) const
{
...
bool asCTokenizer::IsIdentifier(const QChar *source, size_t sourceLength, size_t &tokenLength, eTokenType &tokenType) const
{
...
bool asCTokenizer::IsKeyWord(const QChar *source, size_t sourceLength, size_t &tokenLength, eTokenType &tokenType) const
{
    int start = source[0].unicode();
    if (start > 255)
        return false;
	...
	for( ; *ptr; ++ptr )
	{
		...
		if( sourceLength >= wlen && as_tokencmp(source, (*ptr)->word, wlen) == 0 )
		{
