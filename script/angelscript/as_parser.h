...
public:
	...
	int ParseFunction(asCScriptCode* script);
	int ParseFunctionStatement(asCScriptCode* script);
...
protected:
	...
	void Error(const QString &text, sToken *token);
	void Warning(const QString &text, sToken *token);
	void Info(const QString &text, sToken *token);
	...
	QString ExpectedToken(const char *token);
	QString ExpectedTokens(const char *token1, const char *token2);
