...
JZNodeObject *JZNodeObjectParser::readList(QString valueType,QString gap)
{   
    QString list_type = "QList<" + valueType + ">";
    ...
    JZNodeObject *obj = env->objectManager()->create(list_type);
    if (!obj)
        return nullptr;
    ...
    QVariantList in,out;
    in << QVariant::fromValue(ptr) << QVariant();
    while (1)
    {
...
JZNodeObject *JZNodeObjectParser::readMap(QString keyType,QString valueType)
{
    ...
    JZNodeObject *obj = env->objectManager()->create(map_type);
    if (!obj)
        return nullptr;
    ...
    QVariantList in, out;
    in << QVariant::fromValue(ptr) << QVariant() << QVariant();
    while (1)
    {
        ...
        key = env->convertTo(key, key_type);
        if (!checkVariable(key, key_type))
            return nullptr;
...
{    
    ...
    if (type == "{")
    {
        ...
        if(map)
            return inst->createReference(map->type(),map,true);
        return nullptr;
    }
    else if(type == "[")
    {
        ...
        if(list)
            return inst->createReference(list->type(),list,true);
        return nullptr;
    }
...
QVariant JZNodeObjectParser::readVariable()
{
    ...
    else
    {
        ...
        if (!obj)
            return QVariant();
        return QVariant::fromValue(JZNodeObjectPointer(obj,true));
    }    
...
QString JZNodeObjectFormat::variantToString(const QVariant &v)
{
    ...
    if (JZNodeType::isObject(type))
        ...
        return env->convertTo(v, Type_string).toString();
}
...
/*
    ...
    else
    {
        ...
        {        
            QVariantList in, out;
            in << QVariant::fromValue(JZNodeObjectPointer(obj,false));
            JZScriptInvoke(func_def->fullName(), in, out);
            text += out[0].toString();
        }
        else
