...
asCScriptNode *asCParser::ParseDataType(bool allowVariableType, bool allowAuto)
{
	...
	if( !IsDataType(t1) && !(allowVariableType && t1.type == ttQuestion) && !(allowAuto && t1.type == ttAuto) )
	{
		if( t1.type == ttIdentifier )
		{
			QString errMsg;
			tempString = script->code.mid(t1.pos, t1.length);
			errMsg.sprintf(TXT_IDENTIFIER_s_NOT_DATA_TYPE, qUtf8Printable(tempString));
			Error(errMsg, &t1);
		}
	...
	for(;;)
	{
		...
		else if( t.type == ttCloseParenthesis)
		{
			...
			else
			{
				...
				RewindTo(&t);
				QString str;
				str.sprintf(TXT_UNEXPECTED_TOKEN_s, ")");
				Error(str, &t);
				return node;
			}
		}
...
QString asCParser::ExpectedToken(const char *token)
{
	...
	str.sprintf(TXT_EXPECTED_s, token);
...
QString asCParser::ExpectedTokens(const char *t1, const char *t2)
{
	...
	str.sprintf(TXT_EXPECTED_s_OR_s, t1, t2);
...
QString asCParser::InsteadFound(sToken &t)
{
	...
	if( t.type == ttIdentifier )
	{
		...
		str.sprintf(TXT_INSTEAD_FOUND_IDENTIFIER_s, qUtf8Printable(id));
	}
	else if( t.type >= ttIf )
		str.sprintf(TXT_INSTEAD_FOUND_KEYWORD_s, asCTokenizer::GetDefinition(t.type));
	else
		str.sprintf(TXT_INSTEAD_FOUND_s, asCTokenizer::GetDefinition(t.type));
	...
	while( !isSyntaxError )
	{
		...
		else if( t1.type == ttIdentifier && (IdentifierIs(t1, "repeat") || IdentifierIs(t1, "repeat_same")) )
		{
			if( !isBeginning )
			{
				...
				msg.sprintf(TXT_UNEXPECTED_TOKEN_s, qUtf8Printable(token));
				Error(msg, &t1);
			}
			RewindTo(&t1);
			node->AddChildLast(ParseIdentifier());
...
asCScriptNode *asCParser::ParseArgList(bool withParenthesis)
{
	...
	if( t1.type == ttCloseParenthesis || t1.type == ttCloseBracket )
	{
		if( withParenthesis )
		{
			...
			{
				QString str;
				str.sprintf(TXT_UNEXPECTED_TOKEN_s, asCTokenizer::GetDefinition(ttCloseBracket));
	...
	for(;;)
	{
		while( !isSyntaxError )
		{
			...
			else
			{
				...
				str.sprintf(TXT_UNEXPECTED_TOKEN_s, t);
...
int asCParser::ParseVarInit(asCScriptCode *in_script, asCScriptNode *in_init)
{
	...
	{
		QString msg;
		msg.sprintf(TXT_UNEXPECTED_TOKEN_s, asCTokenizer::GetDefinition(t.type));
		Error(msg, &t);
	}
...
asCScriptNode *asCParser::ParseTypedef()
{
	...
	{
		QString str;
		str.sprintf(TXT_UNEXPECTED_TOKEN_s, asCTokenizer::GetDefinition(token.type));
		Error(str, &token);
		return node;
	}
