...
JZNodeIntegration::JZNodeIntegration(QDesignerFormEditorInterface *core, QObject *parent)
...
void JZNodeIntegration::updateSelection()
{
    if (JZDesignerFormWindow *host = JZDesignerEditor::activeWidgetHost())
        host->updateFormWindowSelectionHandles(true);
    QDesignerIntegration::updateSelection();
}

QWidget *JZNodeIntegration::containerWindow(QWidget * /*widget*/) const
{
    if (JZDesignerFormWindow *host = JZDesignerEditor::activeWidgetHost())
